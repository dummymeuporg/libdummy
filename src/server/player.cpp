...
void Player::setPosition(const std::pair<std::uint16_t, std::uint16_t>& pos)
{
...
void Player::notifyOn(MapUpdatesVector& mapUpdates)
{
    mapUpdates.push_back(
        std::make_unique<Dummy::Protocol::MapUpdate::NamedLivingOn>(
            m_id.value(), serverPosition().first, serverPosition().second,
            character()->floor(), character()->name(), character()->skin(),
            character()->direction()));
}
