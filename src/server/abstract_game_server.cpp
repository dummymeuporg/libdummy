...
AbstractGameServer::AbstractGameServer(boost::asio::io_context& ioContext,
...
void AbstractGameServer::addPending(const std::string& sessionID,
                                    const std::string& accountName)
{
    m_pendingAccounts[sessionID] =
        std::make_shared<Account>(accountName, sessionID);
}

std::shared_ptr<Account>
AbstractGameServer::connect(const std::string& sessionID,
                            const std::string& accountName)
{
    ...
    std::shared_ptr<Account> account = m_pendingAccounts[sessionID];
    m_connectedAccounts[accountName] = account;
    return account;
}

bool AbstractGameServer::isAuthenticated(const std::string& accountName) const
{
    return m_connectedAccounts.find(accountName) != m_connectedAccounts.end();
}

bool AbstractGameServer::isPending(const std::string& id) const
{
    return m_pendingAccounts.find(id) != m_pendingAccounts.end();
}

bool AbstractGameServer::skinExists(const std::string& skin) const
{
    return fs::exists(m_project.projectPath() / "chipsets" / skin);
}

bool AbstractGameServer::characterExists(
    const Dummy::Core::Character& character) const
{
...
void AbstractGameServer::createCharacterFile(
    const Account& account, const Dummy::Core::Character& character) const
{
    ...
    createSymLink((fs::path("..") / "accounts" / account.name() / "characters"
                   / character.filename())
                      .string(),
                  symlinkPath.string());
}

void AbstractGameServer::createSymLink(const std::string& src,
...
void AbstractGameServer::saveCharacter(
    const Account& account, const Dummy::Core::Character& character) const
{
    fs::path characterFullPath(m_serverPath / "accounts" / account.name()
...
void AbstractGameServer::removeSession(std::shared_ptr<GameSession> gameSession)
{
