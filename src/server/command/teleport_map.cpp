#include "dummy/server/command/teleport_map.hpp"

#include "dummy/server/command/command_visitor.hpp"
#include "dummy/server/response/response.hpp"

namespace Dummy {
namespace Server {
namespace Command {

TeleportMap::TeleportMap(const std::string& mapName, tilecoords destination,
                         uint8_t floor, const std::string& instance)
    : m_mapName(mapName)
    , m_destination(destination)
    , m_floor(floor)
    , m_instance(instance)
{}

TeleportMap::TeleportMap(const std::string& mapName, uint16_t x, uint16_t y,
                         uint8_t floor, const std::string& instance)
    : m_mapName(mapName)
    , m_destination(x, y)
    , m_floor(floor)
    , m_instance(instance)
{}

TeleportMap::~TeleportMap() {}

void TeleportMap::accept(
    ::Dummy::Server::Command::CommandVisitor& visitor) const
{
    visitor.visitCommand(*this);
}

} // namespace Command
} // namespace Server
} // namespace Dummy
