...
PlayingState::PlayingState(GameSession& gameSession)
    : State(gameSession)
{}
...
void PlayingState::createMapUpdates(std::shared_ptr<Player> player,
    ...
    for (const auto& [id, living] : m_mapState.livings()) {
        if (map->observers().find(id) == std::end(map->observers())) {
            std::unique_ptr<Dummy::Protocol::MapUpdate::LivingOff> update =
                std::make_unique<Dummy::Protocol::MapUpdate::LivingOff>(id);
...
void PlayingState::visitCommand(const Dummy::Server::Command::Message& message)
{
...
void PlayingState::visitCommand(const Dummy::Server::Command::Ping&)
{
...
void PlayingState::visitCommand(const Dummy::Server::Command::ChangeCharacter&)
{
    ...
    m_gameSession.abstractGameServer().saveCharacter(m_gameSession.account(),
                                                     *player->character());
    ...
    auto response(std::make_shared<Dummy::Server::Response::ChangeCharacter>());
    response->setStatus(0);
                                    ...
                                    const std::string& message)
{
    boost::asio::post(m_gameSession.ioContext(),
                      [author, map = map->shared_from_this(), message]() {
    ...
    const Dummy::Server::Command::TeleportMap& teleportMap)
{
    std::cerr << "Server: teleport to " << teleportMap.mapName() << " ("
              << teleportMap.x() << ", " << teleportMap.y() << ")" << std::endl;
    ...
    if (serverMap == nullptr) {
        // XXX: Throw an exception?
        std::cerr << "Could not access to destination map server." << std::endl;
        m_gameSession.close();
    }
    ...
    Dummy::Protocol::TeleportRequest request(mapName, teleportMap.destination(),
                                             teleportMap.floor(),
                                             instance->name());
    m_gameSession.addResponse(response);
    response->setStatus(0);
