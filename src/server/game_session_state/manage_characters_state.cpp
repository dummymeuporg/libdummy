...
ManageCharactersState::ManageCharactersState(GameSession& gameSession,
                                             CharactersMap&& characters)
    : State(gameSession)
    , m_charactersMap(std::move(characters))
{}
    ...
    const Dummy::Server::Command::SelectCharacter& selectCharacter)
{
    ...
    if (m_charactersMap.find(selectCharacter.name())
        ...
        Dummy::Protocol::TeleportRequest teleportRequest(
            chr->mapLocation(), chr->position(), chr->floor(), "main");
        m_gameSession.changeState(std::make_shared<LoadingState>(
...
{
    Dummy::Server::AbstractGameServer& svr(m_gameSession.abstractGameServer());
    ...
    try {
        std::shared_ptr<Dummy::Core::Character> chr =
            std::make_shared<Dummy::Core::Character>(svr.createCharacter(
                m_gameSession.account(), createCharacter.name(),
        ...
        std::cerr << "Could not create character: " << e.what() << std::endl;
